...
const Navbar = ({ label, route }: NavbarProps) => {
              ...
              <>
                <ul className="lg:!flex sm:hidden font-bold lg:space-x-4 pr-8 max-lg:space-y-2 max-lg:hidden max-lg:py-4 max-lg:w-full">
                  {links.map((link, index) => (
                    <li
                      key={index}
                      className={`max-lg:border-b max-lg:py-2 px-3 text-[#1F1841] hover:text-[#0f3f5d] max-lg:rounded ${
                        index === 0 ? "max-lg:bg-[#2c234d]" : ""
                      }`}
                    >
